"""Clean evaluation across all trained ablation checkpoints with per-token accuracy and clean depth search."""
...
def main() -> None:
    print("=" * 70)
    print("CLEAN EVALUATION OF ALL ABLATION CHECKPOINTS")
    print("=" * 70)
    ...
    res_full_iid = evaluate_clean_model(m_full, test_iid, device=device)
    res_full_held = evaluate_clean_model(m_full, test_heldout, device=device)

    # 2. Sequence-Only Model
    m_seq = VisionToGrammarSequenceOnly(d_model=256).to(device)
    w_seq, t_seq, v_seq = train_variant_clean("Seq-Only", m_seq, ds_train_100, ds_val, epochs=15, has_aux=False, device=device)
    m_seq.load_state_dict(w_seq)
    res_seq_iid = evaluate_clean_model(m_seq, test_iid, device=device, is_seq_only=True)

    # 3. ResNet-18 Model
    m_res = VisionToGrammarResNet(d_model=256).to(device)
    w_res, t_res, v_res = train_variant_clean("ResNet-18", m_res, ds_train_100, ds_val, epochs=15, has_aux=True, device=device)
    m_res.load_state_dict(w_res)
    res_res_iid = evaluate_clean_model(m_res, test_iid, device=device)

    # 4. 50% Data Scale (4 Motifs Preserved)
    m_50 = VisionToGrammarModel(d_model=256).to(device)
    w_50, t_50, v_50 = train_variant_clean("50% Data 4-Motifs", m_50, ds_train_50, ds_val, epochs=15, has_aux=True, device=device)
    m_50.load_state_dict(w_50)
    res_50_iid = evaluate_clean_model(m_50, test_iid, device=device)

    # 5. Single-Motif M1 Model
    m_m1 = VisionToGrammarModel(d_model=256).to(device)
    w_m1, t_m1, v_m1 = train_variant_clean("Single-Motif M1", m_m1, ds_train_m1, ds_val, epochs=15, has_aux=True, device=device)
    ...
    res_m1_seen = evaluate_clean_model(m_m1, ds_m1_seen, device=device)
    res_m1_unseen = evaluate_clean_model(m_m1, ds_m1_unseen, device=device)
    ...
    axes[0, 0].plot(epochs_15, t_seq, label="Train Loss", color="crimson", linestyle="--")
    axes[0, 0].plot(epochs_15, v_seq, label="Val Loss", color="darkred")
    ...
    axes[0, 1].plot(epochs_15, t_res, label="Train Loss", color="blue", linestyle="--")
    axes[0, 1].plot(epochs_15, v_res, label="Val Loss", color="navy")
    ...
    axes[1, 0].plot(epochs_15, v_50, label="50% Data (4 Motifs) Val", color="orange")
    axes[1, 0].plot(epochs_15, v_m1, label="25% Data (M1-Only) Val", color="purple")
    ...
    width = 0.35
    axes[1, 1].bar(x_pos - width/2, oracle_scores, width, label="Oracle True Grammar", color="teal", alpha=0.85)
    axes[1, 1].bar(x_pos + width/2, pred_scores, width, label="Predicted Grammar (Ours)", color="coral", alpha=0.85)
    ...
    print("\n--- Final Clean Benchmark Summary ---")
    print(f"Full Model (IID): Grammar={res_full_iid['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_full_iid['per_token_accuracy']*100:.1f}%, DepthAcc={res_full_iid['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_full_iid['syntactic_validity_rate']*100:.1f}%")
    print(f"Full Model (Heldout): Grammar={res_full_held['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_full_held['per_token_accuracy']*100:.1f}%, DepthAcc={res_full_held['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_full_held['syntactic_validity_rate']*100:.1f}%")
    print(f"Seq-Only (IID): Grammar={res_seq_iid['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_seq_iid['per_token_accuracy']*100:.1f}%, DepthAcc={res_seq_iid['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_seq_iid['syntactic_validity_rate']*100:.1f}%")
    print(f"ResNet-18 (IID): Grammar={res_res_iid['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_res_iid['per_token_accuracy']*100:.1f}%, DepthAcc={res_res_iid['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_res_iid['syntactic_validity_rate']*100:.1f}%")
    print(f"50% Data (IID): Grammar={res_50_iid['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_50_iid['per_token_accuracy']*100:.1f}%, DepthAcc={res_50_iid['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_50_iid['syntactic_validity_rate']*100:.1f}%")
    print(f"Single-Motif Seen: Grammar={res_m1_seen['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_m1_seen['per_token_accuracy']*100:.1f}%, DepthAcc={res_m1_seen['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_m1_seen['syntactic_validity_rate']*100:.1f}%")
    print(f"Single-Motif Unseen: Grammar={res_m1_unseen['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_m1_unseen['per_token_accuracy']*100:.1f}%, DepthAcc={res_m1_unseen['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_m1_unseen['syntactic_validity_rate']*100:.1f}%")
