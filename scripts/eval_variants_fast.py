"""Fast and clean evaluation of remaining ablation variants."""
...
def main() -> None:
    print("=" * 70)
    print("FAST AND VERIFIED ABLATION RUNNER")
    print("=" * 70)
    ...
    w_seq, t_seq, v_seq = train_variant("Seq-Only", m_seq, ds_train_100, ds_val, epochs=10, has_aux=False, device=device)
    m_seq.load_state_dict(w_seq)
    res_seq_iid = evaluate_clean(m_seq, test_iid, device=device, is_seq_only=True)

    # 3. ResNet-18 Model
    m_res = VisionToGrammarResNet(d_model=256).to(device)
    w_res, t_res, v_res = train_variant("ResNet-18", m_res, ds_train_100, ds_val, epochs=10, has_aux=True, device=device)
    m_res.load_state_dict(w_res)
    res_res_iid = evaluate_clean(m_res, test_iid, device=device)

    # 4. 50% Data Scale (4 Motifs)
    m_50 = VisionToGrammarModel(d_model=256).to(device)
    w_50, t_50, v_50 = train_variant("50% Data 4-Motifs", m_50, ds_train_50, ds_val, epochs=10, has_aux=True, device=device)
    m_50.load_state_dict(w_50)
    res_50_iid = evaluate_clean(m_50, test_iid, device=device)

    # 5. Single-Motif M1 Model
    m_m1 = VisionToGrammarModel(d_model=256).to(device)
    w_m1, t_m1, v_m1 = train_variant("Single-Motif M1", m_m1, ds_train_m1, ds_val, epochs=10, has_aux=True, device=device)
    ...
    res_m1_seen = evaluate_clean(m_m1, ds_m1_seen, device=device)
    res_m1_unseen = evaluate_clean(m_m1, ds_m1_unseen, device=device)
    ...
    # Plot Multi-Panel Clean Figures
    ...
    axes[1, 1].bar(x_pos + width/2, pred_scores, width, label="Predicted Grammar (57.4%)", color="coral", alpha=0.85)
    ...
    print("\n" + "=" * 70)
    print("FINAL VERIFIED STAGE 8 ABLATION RESULTS")
    print("=" * 70)
    print(f"Full Multi-Task (IID):  Grammar={res_full_iid['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_full_iid['per_token_accuracy']*100:.1f}%, DepthAcc={res_full_iid['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_full_iid['syntactic_validity_rate']*100:.1f}%")
    print(f"Full Multi-Task (Held): Grammar={res_full_held['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_full_held['per_token_accuracy']*100:.1f}%, DepthAcc={res_full_held['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_full_held['syntactic_validity_rate']*100:.1f}%")
    print(f"Sequence-Only (IID):    Grammar={res_seq_iid['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_seq_iid['per_token_accuracy']*100:.1f}%, DepthAcc={res_seq_iid['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_seq_iid['syntactic_validity_rate']*100:.1f}%")
    print(f"ResNet-18 Backbone:     Grammar={res_res_iid['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_res_iid['per_token_accuracy']*100:.1f}%, DepthAcc={res_res_iid['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_res_iid['syntactic_validity_rate']*100:.1f}%")
    print(f"50% Data Scale (IID):   Grammar={res_50_iid['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_50_iid['per_token_accuracy']*100:.1f}%, DepthAcc={res_50_iid['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_50_iid['syntactic_validity_rate']*100:.1f}%")
    print(f"Single-Motif M1 (Seen): Grammar={res_m1_seen['exact_grammar_match_rate']*100:.1f}%, TokenAcc={res_m1_seen['per_token_accuracy']*100:.1f}%, DepthAcc={res_m1_seen['protocol_b_depth_accuracy']*100:.1f}%, Syntax={res_m1_seen['syntactic_validity_rate']*100:.1f}%")
